...
.Parameter Exportable
	...
	exportable keys.
.Parameter CAFName
	Specifies the Friendly Name of the CA certificate used for signing this certificate
.Example
...
#region Private Key
	...
	# added code for cert siging by another Cert
	if ($IssuerCertFriendlyName) {
		$RootCert = Get-ChildItem Cert:\LocalMachine\My | Where-Object {$_.FriendlyName -eq $IssuerCertFriendlyName}
		
		$signer = New-Object -ComObject X509Enrollment.CSignerCertificate
		$Base64Rootcert = [Convert]::ToBase64String($RootCert.RawData)
		$signer.Initialize(1,0,0x1,$Base64Rootcert)
		$Cert.SignerCertificate = $signer
	}
